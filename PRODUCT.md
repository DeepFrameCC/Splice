...
**brand** — Splice Studio est une boîte de production audiovisuelle. Le site marketing **est** le produit : la qualité visuelle du site fait la première démonstration du savoir-faire. Pas d'app SaaS dashboard derrière (l'espace `/profil` est secondaire). Le design doit refléter l'identité studio créatif, pas la fonctionnalité.

## Product Purpose

Splice Studio produit des contenus audiovisuels pour PME et entreprises du Centre-Val de Loire — pubs sociales, shootings automobile, films de marque, événementiel, intros animées. Le site sert à convertir un prospect en demande de devis qualifiée (tunnel `/devis` 4 étapes, paiement Stripe acompte 30%, livrables sous 14j en moyenne).
...
- **Wedding photographer "minimal pastel"** (Splice Studio n'est PAS un studio mariage)
