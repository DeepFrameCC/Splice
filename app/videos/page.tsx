...
export default async function VideosPage({ searchParams }: { searchParams: Promise<{ owner?: string }> }) {
  ...
  try {
    ...
    const where: any = { type: "VIDEO", published: true };
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-12">
      <header className="mb-8 flex flex-col items-start gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="font-display text-5xl italic text-df-blue">Vidéos</h1>
          <p className="mt-2 text-df-blue/70">Survolez pour la preview, cliquez pour la lecture complète.</p>
        </div>
        <FilterTabs basePath="/videos" current={sp.owner} />
      </header>
      ...
      {dbError ? (
        ...
        <p className="rounded-xl bg-df-cream p-8 text-center text-df-blue/70">Aucune vidéo publiée pour l&apos;instant.</p>
      ) : (
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
          {medias.map((m) => (
            <VideoCard
              ...
              id={m.id}
              src={m.url}
              thumbnail={m.thumbnailUrl ?? "/placeholder-video.jpg"}
