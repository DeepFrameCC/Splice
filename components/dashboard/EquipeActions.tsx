  ...
  return (
    <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-df-blue/10">
      <div className="mb-4 flex items-center gap-3">
        ...
        <div>
          <h2 className="font-display text-lg font-bold text-df-ink">Promouvoir un utilisateur</h2>
          <p className="text-xs text-df-ink/40">L&apos;utilisateur doit avoir un compte existant</p>
        </div>
      </div>

      <form action={handleSubmit} className="flex flex-wrap items-end gap-3">
        <div className="min-w-[200px] flex-1">
          <label htmlFor="invite-email" className="mb-1 block text-xs font-bold text-df-ink/60">
            Email
          </label>
          <input
            id="invite-email"
            ...
            className="w-full rounded-xl border border-df-blue/15 px-4 py-2.5 text-sm text-df-ink outline-none transition focus:border-df-blue focus:ring-2 focus:ring-df-blue/20"
          />
        </div>
        <div>
          <label htmlFor="invite-role" className="mb-1 block text-xs font-bold text-df-ink/60">
            Rôle
          </label>
          <select
            id="invite-role"
            name="role"
            className="rounded-xl border border-df-blue/15 px-4 py-2.5 text-sm text-df-ink outline-none transition focus:border-df-blue focus:ring-2 focus:ring-df-blue/20"
          >
      ...
      {message && (
        <p className={`mt-3 text-sm font-bold ${message.type === "error" ? "text-red-600" : "text-emerald-600"}`}>
          {message.text}
        </p>
      )}
    </div>
...
export function TeamMemberCard({ member, isSelf }: { member: TeamMember; isSelf: boolean }) {
  ...
  return (
    <div className="relative rounded-2xl bg-white p-5 shadow-sm ring-1 ring-df-blue/10 transition hover:shadow-md">
      {isAdmin && (
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          {fullName && (
            <p className="truncate text-xs text-df-ink/50">{fullName}</p>
          )}
          <p className="truncate text-xs text-df-ink/40">{member.email}</p>
        </div>
      </div>
        ...
        {!isSelf && (
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={isPending}
              className="inline-flex items-center gap-1 rounded-lg px-2.5 py-1 text-[11px] font-bold text-red-600 transition hover:bg-red-50 disabled:opacity-50"
            >
      ...
      {isSelf && <p className="mt-2 text-[10px] text-df-ink/30">Vous ne pouvez pas modifier votre propre rôle.</p>}
    </div>
  );
}
